...
const onError: ErrorHandler = (err, c) => {
  ...
  // isTruthyFlag, same as the logger and docs gates — a raw === "true" compare
  // here made DEBUG=1 enable debug logs but not error details (split-brain).
  const debug = isTruthyFlag((c.env as { DEBUG?: string } | undefined)?.DEBUG);
