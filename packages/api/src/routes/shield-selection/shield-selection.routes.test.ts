...
const ROUTES: [string, string, string][] = [
  ["getMine", "get", "/shield"],
  ["upsertIos", "post", "/shield/ios"],
  ["upsertAndroid", "post", "/shield/android"],
  ["setWindows", "post", "/shield/windows"],
  ["setEnabled", "post", "/shield/enabled"],
];
...
describe("shield-selection route contract", () => {
  it.each(
    ROUTES
  ...
  it('tags every route under "ShieldSelection"', () => {
    for (const [name] of ROUTES) {
      expect(routeMap[name].tags).toContain("Shield");
    }
  });
