...
const ROUTES: [string, string, string][] = [
  ...
  ["claimPolarByEmail", "post", "/subscription/claim-polar"],
  ["claimMockSubscription", "post", "/subscription/claim-mock"],
  ["syncRevenueCat", "post", "/subscription/revenuecat"],
];
...
describe("subscriptions route contract", () => {
  it.each(
    ROUTES
  ...
  it('tags every route under "Subscriptions"', () => {
    for (const [name] of ROUTES) {
      expect(routeMap[name].tags).toContain("Subscriptions");
    }
  });
