...
const ROUTES: [string, string, string][] = [
  ["getCachedPrayerTimes", "get", "/prayer-times"],
  ["refreshPrayerTimes", "post", "/prayer-times/refresh"],
];
...
describe("prayer-times route contract", () => {
  it.each(
    ROUTES
  ...
  it('tags every route under "PrayerTimes"', () => {
    for (const [name] of ROUTES) {
      expect(routeMap[name].tags).toContain("PrayerTimes");
    }
  });
