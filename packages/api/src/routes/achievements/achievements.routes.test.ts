...
const ROUTES: [string, string, string][] = [
  ["listForMe", "get", "/achievements"],
  ["listUnseen", "get", "/achievements/unseen"],
  ["markSeen", "post", "/achievements/seen"],
];
...
describe("achievements route contract", () => {
  it.each(
    ROUTES
  ...
  it('tags every route under "Achievements"', () => {
    for (const [name] of ROUTES) {
      expect(routeMap[name].tags).toContain("Achievements");
    }
  });
