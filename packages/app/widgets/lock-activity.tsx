...
function LockActivityLayout(
  ...
  // Live Activities render on the Lock Screen / Dynamic Island / StandBy, which
  // have no Clear or Tinted appearance (those are Home Screen icon+widget
  // modes) — `LiveActivityEnvironment` accordingly carries no
  // `widgetRenderingMode`. The two signals that do exist are the surrounding
  // colour scheme and StandBy's reduced-luminance state, so adapt to those
  // instead of hardcoding one palette.
  const scheme = environment.colorScheme ?? "dark";
  const dimmed = environment.isLuminanceReduced === true;
  // On a light banner the cream ink disappears, so flip to the deep ink brown.
  // Under reduced luminance everything steps down a stop: StandBy night mode
  // asks for lower overall brightness.
  let CREAM = "#f5ebdb";
  let CREAM_DIM = "#b9ad92";
  if (scheme === "light") {
    CREAM = "#1a1408";
    CREAM_DIM = "#1a14088c";
  }
  if (dimmed) {
    CREAM = scheme === "light" ? "#1a140899" : "#f5ebdbb3";
    CREAM_DIM = scheme === "light" ? "#1a140866" : "#b9ad9280";
  }
  const GREEN = dimmed ? "#29603e99" : "#29603e";
  ...
  const HAIRLINE = scheme === "light" ? "#1a140840" : "#b9ad9259";
  const ICON =
